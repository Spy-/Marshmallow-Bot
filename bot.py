...
from utilities.dataIO import dataIO
from utilities.settings import Settings
from utilities.chat_formatting import inline
...
class Bot(commands.Bot):
    ...
    async def send_cmd_help(self, ctx, command):
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    bot = bot_class(formatter=formatter,
                    description="All the commands for this bot.", pm_help=None)
    ...
    async def on_command_error(error, ctx):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            await bot.formatter.format_help_for(ctx, ctx.command, "Missing required arguments")
...
def check_folders():
    folders = ("data/bot", "logs", "cogs", "utils")
...
def interactive_setup(settings):
    ...
    if first_run:
        ...
        print("\nThe configuration is done. Leave this window always open to"
              ...
              "Please read this guide for a good overview on how the bot works:\n"
              "https://github.com/Spy-/discord-bot-2.0\n"
...
def set_cog(cog, value):
    data = dataIO.load_json("data/bot/cogs.json")
    data[cog] = value
    dataIO.save_json("data/bot/cogs.json", data)
...
def load_cogs(bot):
    ...
    try:
        registry = dataIO.load_json("data/bot/cogs.json")
    except:
        registry = {}
    ...
    if bot.settings._no_cogs:
        ...
        if not os.path.isfile("data/bot/cogs.json"):
    ...
    dataIO.save_json("data/bot/cogs.json", registry)
