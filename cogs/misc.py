...
class Miscallaneous:
    ...
    async def help(self, ctx, *, command=None):
        ...
        if command is None:
            ...
            subcommands = [x.qualified_name for x in utils.get_all_subcommands(
                command) if x != command]

            # The rest is simple, create the embed, set the thumbail to me, add all fields if they exist
            embed = discord.Embed(title=command.qualified_name, colour=discord.Colour.gold())
    ...
    async def botinfo(self, ctx):
        ...
        description = "A bot made by Spyn"

        # Now creat the object
        opts = {'title': 'Marshmallow',
    ...
    async def roll(self, ctx, *, notation: str = "d6"):
        ...
        nums = [random.SystemRandom().randint(1, num)
