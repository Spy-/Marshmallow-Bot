...
class Music:
    ...
    async def join_channel(self, channel, text_channel):
        ...
        log.info("Joining channel {} in guild {}".format(
    ...
    async def progress(self, ctx):
        ...
        if state is None or not state.playing or state.current is None:
            ...
            fmt = "Current song progress: {0[0]}m {0[1]}s/{1[0]}m {1[1]}s".format(
                progress, length)
            await ctx.send(fmt)
    ...
    async def _import(self, ctx, *, song: str):
        ...
        if not config.playlist_allowed:
        ...
        if state.user_queue:
            await ctx.send("The current queue type is the DJ queue. Use the command {}dj to join this queue".format(ctx.prefix))
            return
    ...
    async def stop(self, ctx):
        ...
        # So our workaround here is try to connect (this may timeout, and force it to now be visually disconnected) then disconnect
            ...
            if ctx.message.guild.me.voice:
                ...
                channels = [c for c in ctx.message.guild.voice_channels if c.permissions_for(
                    ctx.message.guild.me).connect]
                if len(channels) > 0:
                    channel = channels[0]
    ...
    async def queue(self, ctx):
        ...
        if len(_queue) == 0:
            ...
            self.bot.loop.create_task(self.queue_embed_task(
                state, ctx.message.channel, ctx.message.author))
    ...
    async def playing(self, ctx):
        ...
        if state is None or not state.playing or not state.current:
            ...
            embed.add_field(
                name='Title', value=state.current.title, inline=False)
            if state.current.requester:
                embed.add_field(
                    name='Requester', value=state.current.requester.display_name, inline=False)
            # Get the amount of current skips, and display how many have been skipped/how many required
            skip_count = len(state.skip_votes)
            embed.add_field(
                name='Skip Count', value='{}/{}'.format(skip_count, state.required_skips), inline=False)
            ...
            if length and progress:
                progress = divmod(round(progress, 0), 60)
                length = divmod(round(length, 0), 60)
                fmt = "{0[0]}m {0[1]}s/{1[0]}m {1[1]}s".format(
                    progress, length)
