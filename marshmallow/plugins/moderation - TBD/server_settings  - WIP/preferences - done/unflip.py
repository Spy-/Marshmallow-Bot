...
async def unflip(cmd, message, args):
    if message.author.permissions_in(message.channel).manage_guild:
        flip_settings = cmd.db.get_guild_settings(message.guild.id, 'Unflip')
        if flip_settings is None:
            unflip_set = False
        else:
            unflip_set = flip_settings
        if unflip_set:
            cmd.db.set_guild_settings(message.guild.id, 'Unflip', False)
            response_title = f'✅ Table unflipping has been **disabled**.'
            response = discord.Embed(color=0x77B255, title=response_title)
        else:
            cmd.db.set_guild_settings(message.guild.id, 'Unflip', True)
